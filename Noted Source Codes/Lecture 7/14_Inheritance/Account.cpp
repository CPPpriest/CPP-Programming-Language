...
Account::Account(string oName) : mAccountNumber(sNextAccountNumber),
                                 ...
                                 mLog("")
{
    sNextAccountNumber++;   // first increment, then use as key
                            // so the first created account has the number 1, not 0

    sAccounts[mAccountNumber] = this;
    /* Adds Account address to map
     * since Account and PremiumAccount have the same address
     * and can use the same pointers; nothing needs to be changed.
     */
